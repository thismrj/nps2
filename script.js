const Slider = {
    ...
    next: function () {
        ...
        const [mquery, cls] = this.media[this.current.branch];

        if (window.matchMedia(mquery).matches)
            document.querySelector('form.card').classList.add(cls)
        // else
            // document.querySelector('form.card').classList.remove(cls)
    },
    update: function () {
